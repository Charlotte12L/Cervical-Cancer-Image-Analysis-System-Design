...
ResultAddress = '../StatsResults/PATHOLOGY_A/'
...
for i_modality_index in np.arange(5):
   ...
   # print(feature_arrayA0)
    ...
    for train, test in cv.split(feature_array, group_inf):   #40折，test为1，trian为39   array([0])   
        ...
        a = clf.predict(test_DataX)#测试集的预测结果  test_score_record[A1]=[[0.5503842 0.4496158]] 前者为0 后者为1
        test_score_record[patient[test[0]]] = clf.predict(test_DataX)[0]  #test[0]即为训练集的标号，本次为0，实时记录本次预测的值.[0][1]
    ...
    ################################################################################################################
    bit_score = np.array(np.array(test_score) > 0.45).astype(int)  #大于0.5则标记为1   test_score大小为（40，）
